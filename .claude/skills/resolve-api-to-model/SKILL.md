...
*   For each field, determine:
    ...
    *   **Required**: Check if the field is required in *at least one* mode (this affects validation, not property declaration).
    ...
    if (this.formMode === 'create') {
      return object({
        username: string()
          ...
          .matches(CommonRegexPattern.USERNAME_PATTERN, '使用者名稱只能包含英文、數字和底線'),
        email: string()
          .required('電子郵件為必填')
          .email('請輸入有效的電子郵件'),
        password: string()
          .required('密碼為必填')
          .min(8, '密碼至少需要 8 個字元')
          .matches(CommonRegexPattern.PASSWORD_PATTERN, '密碼必須包含大小寫字母和數字'),
      });
    }
    else if (this.formMode === 'edit') {
      return object({
